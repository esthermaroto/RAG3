...
def run_evaluator(evaluator_path, new_title, description, ollama=True):
    """
    ...
    try:
        ...
        if ollama:
            ...
            # Load the configuration from environment variables for Github Models
            configuration = {
...
def main(ollama=True):
    ...
    for evaluator_file in evaluator_files:
        ...
        start_time = time.time()
        result = run_evaluator(evaluator_file, new_title,
    ...
    print(f"Now let's run the same with Github Models 🚀")
    start_time = time.time()

    print(
        f"[bold blue]Model for title generation 📝[/bold blue]: {os.getenv('GITHUB_MODELS_MODEL_FOR_TEXT_GENERATION')}")
    # Get the new title
    new_title = prompty.execute(
        Path.cwd() / "prompt-engineering/generate_title.prompty",
        inputs={"description": description},
        configuration={
            "name": os.getenv('GITHUB_MODELS_MODEL_FOR_TEXT_GENERATION'),
            "type": "openai",
            "base_url": os.getenv('GITHUB_MODELS_API_URL'),
            "api_key": os.getenv('GITHUB_MODELS_API_KEY')
        }
    ...
    print(
        f"Evaluator using [bold green]{os.getenv('GITHUB_MODELS_MODEL')}[/bold green]")
    ...
    for evaluator_path in evaluator_files:
        ...
        start_time = time.time()
        result = run_evaluator(evaluator_path, new_title,
                               description, ollama=False)
        end_time = time.time()
        elapsed_time = end_time - start_time
        total_elapsed_time += elapsed_time
        print(
            f"Evaluator '{evaluator_path.name}' executed in {elapsed_time / 60:.2f} minutes 🕒"
        )
...
if __name__ == "__main__":
    # Pass True for Ollama or False for GitHub Models
    main(ollama=True)
